...
担当スタッフ制を入れたのに、**チャットだけがその前の形**で残っていた。
お客様の宛先は代表スタッフ1名（`fetchMyTenantTrainerId`）に固定で:
...
`sender_id` / `receiver_id` の1対1のまま。**読むときにスタッフ全員を
ひとまとまりとして扱う**だけ。マイグレーション0件、データ移行なし。

```
ジム側   useMessages(customerId, { selfIds:  staff.ids })
お客様側 useMessages(trainerId,  { otherIds: staff.ids })
```

テナント内でスタッフが全メッセージを読めることは、既存の RLS
（`"Trainers can view all messages"` ＋ RESTRICTIVE な `tenant_isolation`）で
**もともと許可されていた**。足りなかったのはクライアントの絞り込みだけだった。

### 外してはいけない点

| | 外すとどうなるか |
|---|---|
| 絞り込みは行き・帰り**両方**を `in` にする | 片方が `eq` だと担当以外の返信が会話から抜け落ちる |
| 相手側から自分を除く | スタッフが自分自身を相手側に持ち「自分との会話」が混ざる |
| 未読はこちら側の**集合**で数える | 別スタッフ宛ての未読が出ず、誰も気づかない |
| 未読からスタッフ同士の行を除く | スタッフの返信を「お客様からの新着」と数える |
| 既読もこちら側の集合を対象に | 開いて読んだのに未読が消えない |
| 吹き出しの左右は「お客様かどうか」で決める | 他スタッフの返信が**お客様側**に並ぶ |

### 🔴 スタッフ名は `tenant_members.display_name` から取る

`profiles` はお客様から他人の行を読めない（`profiles_tenant_scope_select`）。
直接 `profiles` を引くと**お客様側でだけ名前が出ない**。
既存の `useTenantStaff`（→ `src/lib/tenantStaff.ts`）を通すこと。
`useStaffDirectory` はその薄いラッパー。

> ⚠️ この実装中に、既存の `useTenantStaff.ts` を同名で作り直して**上書きしかけた**。
> 既存フックがすでに必要なものを持っていたので、ラッパーに切り替えた。

お客様側で名前を出すのは**スタッフが2人以上のときだけ**（1人ジムでは毎回同じ名前が並ぶだけ）。

---

## LINE 相当の使い勝手（2026-08-12）

「メッセージ順に並べて LINE みたいに」の依頼で入れたもの。

| | 内容 | DB |
|---|---|---|
| 会話一覧の並び | 最後にやり取りした順 | 変更なし |
| 日付の区切り | 「今日」「昨日」「8/10(月)」 | 変更なし |
| URL のリンク化 | 本文の `http(s)` だけ押せる | 変更なし |
| 画像の全画面表示 | アプリ内のライトボックス | 変更なし |
| 引用返信 | 長押し → 返信 | 変更なし |
| 会話内の検索 | 読み込み済みを絞る | 変更なし |
| 送信取り消し | 24時間・送信者本人 | `messages.unsent_at` |
| リアクション | Lucide アイコン4種 | `message_reactions` |

### 🔴 会話一覧は「未読優先」にしない

以前は未読がある人を上にしていた。**返信しようとして開く＝既読にする**ので、
読んだ瞬間にその会話が一覧の下へ飛び、返信するのに探し直すことになっていた。
未読は新着なので時刻順にすれば自然に上に来る（バッジでも分かる）。

### 🔴 本文を HTML として描画しない

リンク化も検索の強調も、**文字列を分割して React 要素として組む**。
本文はお客様が自由に入力できるので、`dangerouslySetInnerHTML` を通すと
そこがスクリプト実行の入り口になる。`http`/`https` 以外はリンクにせず、
**弾いた文字列も本文からは消さない**（送った本人の文が黙って欠ける）。

### 🔴 引用は参照ではなく文字列

`reply_to_id` を持たせると**送信取り消しと噛み合わない**。元を取り消したあとに
「削除されたメッセージ」の枠だけが残り、しかも**取り消したはずの本文が
引用の中に生き残る**。予約の引用と同じ方式に揃えた。

裏を返すと「引用されたあとは取り消しても抜粋が残る」。24時間の猶予は
**引用される前に間に合わせる**ためのものでもある。

### 🔴 受信者が「届いた本文」を書き換えられた（塞いだ）

送信取り消しを入れるために UPDATE の経路を調べて見つかった**既存の穴**。
本番で再現している:
...
2. UPDATE ポリシー名は "Receiver can update read status" なのに **WITH CHECK が無い**
   （USING は「自分が受信者か」しか見ない＝どの列を変えてもよい）

RLS では「どの列を変えたか」を書きにくいので、**列レベルの GRANT** で締めた。
クライアントが直接 UPDATE してよいのは `read` だけ。取り消しは RPC を通す。
...
⚠️ ここを `GRANT UPDATE (read, content)` のように広げると穴が開き直る。
`messageUnsend.test.ts` が見張っている。

### 送信取り消しで決めたこと

| | 外すとどうなるか |
|---|---|
| 行ごと消さない（「取り消しました」を残す） | 吹き出しが丸ごと消え「何か言ったはずなのに無い」になる |
| 本文と添付を **DB から実際に落とす** | 表示で隠すだけでは取り消したことにならない |
| 添付はストレージからも消す | 参照は外れるがファイルが残る |
| 可否の判断は **RPC（SECURITY DEFINER）** | 端末の時計はずれる。直接叩かれれば素通り |
| 他のスタッフの発言は取り消せない | 誰が消したのか分からなくなる |

⚠️ **すでに飛んだプッシュ通知は取り消せない**（LINE と同じ制約）。
UI の文言で過信させないこと。

### リアクションで決めたこと

- **絵文字を使わない**（リポジトリの規約）。DB には種別のキーだけを入れ、
  描画側が Lucide のアイコンに割り当てる
- 🔴 **通知を鳴らさない。** `messages` の INSERT ではないのでトリガーの対象外。
  気軽に押せることが価値なので、ここに通知を足さない
- 見える範囲は `messages` の見える範囲に**そのまま乗せる**（EXISTS で引く）。
  独自条件を書くと messages 側を直したときに食い違う。
  **`shares_tenant_with_me` に緩めない**（同じジムの別のお客様に見えてしまう）
- 並びは種別の定義順に固定。件数順にすると**押すたびにチップが入れ替わる**
...
## 🔴 入力欄がキーボードに隠れる — 3回目でようやく（2026-09-03）

同じ不具合を2回「直した」つもりで、2回とも iOS の実機では直っていなかった。

| いつ | 何をした | 結果 |
|---|---|---|
| 9/01 (#364) | `100vh` をやめ、`visualViewport` から `--kb` を出して下端に当てた | Android で直った。**iOS は直っていない** |
| 9/02 (#365) | 下端の逃がしを 4rem 直書き → ナビの実測値（`--nav-h`）に | Android の別症状が直った。**iOS は直っていない** |
| 9/03 (#380) | 足切りの位置を直した（下記） | 実機で未確認 |

### 何が間違っていたか

```ts
const overlap = innerHeight - (viewportHeight + offsetTop);
if (overlap < MIN_KEYBOARD_PX) return 0;   // ← ここ
```

`- offsetTop` 自体は**幾何的に正しい補正**。`position: fixed` はレイアウトビューポート
基準に置かれるので、iOS がビジュアルビューポートをずらしているぶんは持ち上げから引く。

間違っていたのは**足切りをかける対象**。iOS はキーボードを出すとき、フォーカスした入力欄を
見せようとビジュアルビューポートをずらす。入力欄は画面のいちばん下にあるので、ずれる量は
キーボードの高さに近くなる。すると引いたあとの値が 80px を下回って足切りに落ち、
**持ち上げが丸ごと 0 になって入力欄がキーボードの裏へ戻る。**

足切りは「キーボードが出ているか」（＝ `covered = innerHeight - viewportHeight`）の
判定に使うもので、持ち上げ量に使うものではなかった。

### 🔴 「Android で直った」は iOS の証拠にならなかった

Android は `adjustResize` で **WebView ごと縮む**ので `covered` も `offsetTop` も 0 になり、
**そもそもこの計算を通らない**。直った理由が別だったのに、同じ式が効いていると解釈して
2回とも iOS を確認しないまま出荷した。**片方のプラットフォームで直っても、
もう片方の証拠にはならない。**

### 数字を見られるようにした（`?kb=1`）

この不具合は**クラウドのセッションからは1度も再現できない**（jsdom に `visualViewport` が
無く、ネイティブも動かせない）。こちらは数式を読んで推測するしかなく、実機を持っている人は
「隠れている」以上を返せない。その往復が2回空振りした。

`src/components/KeyboardMetrics.tsx` — URL に `?kb=1` を付けたときだけ、
`inner / vh / off / covered / kb` を画面の左上に出す。既定では何も描かない。
