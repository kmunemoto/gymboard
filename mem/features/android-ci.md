...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。iOS は次 **1.7.0**（未使用のまま。中身は PR #382〜#389 ＝ ブロック枠の用事名／性別の引っ越し／ゲーム要素の撤去／当日の空き状況を予約者だけに見せる／npm install の再試行）。⚠️ **Android は 8/29 以降ビルドしていない。** #363（自宅ストレッチ動画）・#364（1日の受付上限とワンタップ受付停止／チャット入力欄がボトムナビに隠れる修正）・#365〜#373（1.6.7 の中身）・#374〜#379（1.6.8 / 1.6.9 の中身）・#380（スタンプ）・#382〜#389 が**すべて未到達**。次に Android を出すときのノートは iOS のものと中身が大きく違う |
