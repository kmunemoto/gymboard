...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。🔴 **Android には #302 以降がすべて未到達**（8/12 以降ビルドしていない）。iOS は 1.5.8。PR #314〜#317（エアリザーブ由来の予約設定5件・営業時間の選べる範囲拡大・24時間営業対応）は**まだどちらのビルドにも載っていない** |
