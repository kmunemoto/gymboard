...
| 2026-08-13 | — | — | **iOS 1.5.4 が承認・公開済みであることが Apple の 409 で判明**（Actions #130 が `The train version '1.5.4' is closed` / `previously approved version [1.5.4]` で拒否された）。⚠️ **1.5.4 は2回アップロードされている**: ビルド **128**（`4dfc0f8`＝PR #301 まで。チャット改修は入っていない）と ビルド **129**（`1f302db`＝PR #302 込み）。**どちらが承認されたかで中身が違う。** App Store Connect で 1.5.4 のビルド番号を確認すること。次は **1.5.5**（Actions #130 で消費できなかったので 612811b の内容は未出荷） |
| （未リリース） | 86 | 9.5 | `android-version.json` の現在値。**Android は 9.5 をまだ Play に上げていない。** iOS の次は **1.5.5**（1.5.4 は承認済みで再利用できない）。1.5.5 に載るのは PR #302（チャットの LINE 化）と #303（オーナーのアカウント削除・npm 固定）。⚠️ **#302 は 1.5.4 のビルド129に入っている可能性がある**（上の 2026-08-13 の行を参照） |
