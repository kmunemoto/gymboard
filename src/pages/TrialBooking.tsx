...
import { getJSTNow, getJSTToday, toJSTDate } from "@/lib/timezone";
...
const TrialBooking = () => {
  ...
  // 受付を終了した日（手で閉めた日＋1日の上限に達した日）。公開ページなので
  // 匿名でも読める RPC（get_tenant_closed_days）から取る。最終判定は DB（GB007）。
  // 会員アプリと**同じ答え**を同じ経路で読むので、両画面で食い違わない。
