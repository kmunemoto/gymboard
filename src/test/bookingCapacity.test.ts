...
//   2. 時間帯は [start, end) の半開区間。⚠️ 予約回数の制限（bookingLimits）は
//      2026-08-21 から閉区間 [start, end] で、**意図的に違う**。帯は「その時間に
//      居るスタッフの数」なので、帯が終わった時刻に始まる予約に効かせてはならない
//      （12時に帰る応援の帯が 12:00 開始の枠を受けてしまう）。制限のほうは
//      「開始時刻の範囲」で、店の「〜19:00」は 19:00 の回まで、と読まれる
