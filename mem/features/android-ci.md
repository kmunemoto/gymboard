...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない（2026-08-13 に `android-version.json` を削除）。Android Studio で `android/app/build.gradle` を直接上げる。🔴 **Android には #302（チャットのLINE化）・#303（アカウント削除）・#308（運営への要望）・#310/#311（営業時間の反映）がすべて未到達。** iOS の次は **1.5.6**（PR #308 / #310 / #311）|
