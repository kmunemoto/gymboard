...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。🔴 **Android には #302 以降がすべて未到達**（8/12 以降ビルドしていない）。**次に Android を出すと、iOS の 1.5.3〜1.6.0 の全部が一度に入る＝リリースノートは iOS のものと中身が違う**（チャットのLINE化・オーナーのアカウント削除・予約設定5件・アイコン刷新・予約の制限3種・受付しない時間帯・通知のサーバー側移行・設定のカテゴリー分け ほか）。iOS は次 1.6.1。⚠️ **アイコンの刷新（#319）は Android では AAB を作り直さないと反映されない**（`npx @capacitor/assets@3 generate --android` → AAB → Play Console）。ストア掲載アイコン 512×512 とフィーチャーグラフィックは Play Console の Web UI から別途手動アップロード |
