 ...
 *
 * ⚠️ **他の機能フラグと違い、OFF にしても画面が完全には消えない。**
 * チラシやQRに印刷済みのリンクを 404 にしないため、経路を残して案内文に差し替えている。
 * つまり `trialBooking.notAvailableTitle` / `notAvailableBody` は
 * **OFF のフォークでも実際にお客様の目に触れる**。既定文は「ジムまでお問い合わせください」
 * なので、フォークは業種オーバーレイでこの2キーを必ず差し替えること
 * （セッコツボードが実際に踏んで報告・2026-08-02。プリセット側にも追記済み）。
 */
