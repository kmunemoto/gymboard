...
| 2026-09-03 | — | — | **iOS 1.6.9（Actions #148 = `a2b1eaa`・9/3 07:17Z）がリリース済み**（「リリースノートだけ書いて」の合図で実績化）。⚠️ **前回の記録以降、iOS は2回上がっている**: **1.6.8** ＝ Actions **#147** = `4e36365`（9/2 19:59Z）と **1.6.9** ＝ Actions **#148** = `a2b1eaa`（9/3 07:17Z）。**1.6.8 が単独でお客様に届いたかはセッションからは分からない**（7時間後に 1.6.9 が上がっている）。**出ているのは 1.6.9** として扱う。中身は 1.6.7 以降のすべて＝ PR #374〜#379: 🔴 **#375＝予約オプションが保存できない不具合の修正**（1.6.7 では、店側でオプションを追加して保存しても、同じ保存処理が入れた直後の行を消していた。本番の `booking_options` は全テナントで0件だった。**オプションが実際に使えるようになったのはこの版から**）／🔴 **#379＝iOS でチャットの入力欄がキーボードの裏に隠れる不具合の修正**（Android では 9/1 の版で直っていたが、iOS だけ残っていた。`--kb` の計算で clamp を掛ける位置が違い、`offsetTop` を引く前に0で止めていた）＋ 予約が断られたときに「アプリが古い」場合の案内を出す（未知の `GB0xx` を検出）／**#376＝予約オプションを、枠を選んだあとの確認カードで毎回聞く**（それまでは枠グリッドの手前にあり、お客様が気づけなかった。後ろが埋まっていたら「21:00 に変更して付ける」ボタンを出す）。#374 / #377 / #378 は記録と兄弟アプリ向けのプロンプトのみで、お客様から見た変化は無い。`MARKETING_VERSION` を **1.7.0** に進めた。⚠️ **チャットのスタンプ（PR #380）はこの版に入っていない。** 次のビルドで出る （🔴 **この警告は誤り。** 同じ 1.6.9 で Actions **#149** が 9/3 12:48Z に上がっており、スタンプはそちらに入っている。下の 2026-09-05 の行を見ること） |
| 2026-09-05 | — | — | **iOS 1.6.9 = Actions #149（`cacae7d`・9/3 12:48Z）がリリース済み**（「リリースノート作って」の合図で実績化）。アップロードはログの `UPLOAD SUCCEEDED with no errors` / Delivery UUID `da37a81b-4fb1-46ca-9444-8bcef5442727` で確認（緑だけで判断していない）。中身は **PR #380 ＝ チャットのスタンプ**（ジムボード公式キャラのスタンプ8枚。絵はアプリに同梱し、DB は `messages.sticker_id` だけ持つ）。🔴 **1.6.9 は2回上がっている**: **#148** = `a2b1eaa`（07:17Z、スタンプ無し）と **#149** = `cacae7d`（12:48Z、スタンプ入り）。前回（9/3）の記録は #148 を出荷版と見て「スタンプは入っていない」と書いたが、**合図の直前に上がっているのは #149**（合図は 9/4 00:31 JST ＝ #149 の約2時間半後）。従来どおり「合図の直前に上がった版が出荷版」の判断に揃え、**スタンプは 1.6.9 で出た**として扱う。⚠️ **`MARKETING_VERSION` はここでは上げない。1.7.0 のままでよい。** 1.7.0 で走ったのは Actions **#150**（`dedb489`・9/5 13:05Z）だけで、**`Install dependencies` で落ちて `Upload to App Store Connect` は skipped**＝App Store Connect には一度も上がっていない（原因は `npm error code ECONNRESET` の一過性。#388 で3回まで再試行するようにした）。⚠️ **`cacae7d` 以降はすべて未出荷**: #382（ブロック枠の用事名）・#383（予定名を客に見せない番人）・#384/#385（性別を profiles へ移す）・#386/#387（ゲーム要素の撤去）・#388（npm install の再試行）・#389（当日の空き状況を予約者だけに見せる）。⚠️ **Android は 8/29 以降ビルドしていない**（#363 以降がすべて未到達） |
| 2026-09-08 | — | — | **iOS 1.7.0 = Actions #152（`3295395`・9/5 15:02Z）がリリース済み**（「リリースノート考えて」の合図で実績化）。アップロードはログの `UPLOAD SUCCEEDED with no errors` / Delivery UUID `14ba929b-c395-45a6-afe3-723ff3f78fea` で確認（緑だけで判断していない）。⚠️ **1.7.0 は2回上がっている**: **#151** = `52fcc6e`（14:17Z）と **#152** = `3295395`（15:02Z）。差は #390（記録のみ）だけなので中身は同じ。中身は 1.6.9 以降のすべて＝ PR #382〜#390: **#389＝1日の上限で埋まった当日の空き状況を、その日に予約しているお客様にだけ見せる**（お客様から見ていちばん大きい）／**#386・#387＝ゲーム要素の撤去**（アバター・ガチャ・クエスト等。テーブル50個・関数55本・列2つを本番から削除。予約788件・記録2,434件は1件も減っていない）／**#382＝予定表のブロック枠に用事名**（🔴 店内だけ。お客様には従来どおり「満枠」としか見えない）／#384・#385（性別を profiles へ移す）・#388（npm install の再試行）は内部のみ。🔴 **`MARKETING_VERSION` はここでは上げない。** 宗本さんが 9/8 に `ios-build.yml` を直接 1.7.0 → **1.7.1** に編集済み（`2a1c406`）。⚠️ **1.7.1 = Actions #153（`2a1c406`・9/8 00:56Z）はアップロード済みだが、公開は未確認**（ログの `No errors uploading archive` で成功は確認）。中身は **PR #391 ＝ アプリ更新のお願いダイアログ**のみ。🔴 **本番の `app_releases.latest_version` は 1.7.0 に更新した**（1.7.1 ではない。公開が確認できていない版を入れると、存在しないかもしれない版への更新を促すことになる。公開が分かったら 1.7.1 に上げること）。⚠️ **更新ダイアログは 1.7.1 では誰にも出ない**。1.7.1 の人が最新だから。**その次の版から効き始める**。⚠️ **Android は 8/29 以降ビルドしていない**（#363 以降がすべて未到達） |
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。iOS は次 **1.7.1**（宗本さんが設定済み。Actions #153 でアップロード済み・公開待ち。中身は PR #391 ＝ アプリ更新のお願いダイアログ）。🔴 **1.7.1 の公開が分かったら `app_releases.latest_version` を 1.7.1 に上げること**（`mem/features/app-update-prompt.md`）。上げて初めて、1.7.0 のまま止まっている人に更新の案内が出る。⚠️ **Android は 8/29 以降ビルドしていない。** #363（自宅ストレッチ動画）・#364（1日の受付上限とワンタップ受付停止／チャット入力欄がボトムナビに隠れる修正）・#365〜#373（1.6.7 の中身）・#374〜#379（1.6.8 / 1.6.9 の中身）・#380（スタンプ）・#382〜#391 が**すべて未到達**。次に Android を出すときのノートは iOS のものと中身が大きく違う |

### 🔴 npm install の一過性の失敗でビルドを落とさない（2026-09-05）

Actions **#150** が `npm error code ECONNRESET` で **32秒**で失敗した。
コードは無関係で、npm レジストリへの接続が途中で切れただけ。

**ネットワークが1秒詰まっただけでビルド1回ぶんを失う**ので、
`ios-build.yml` の依存インストールを3回まで再試行するようにした（15秒→30秒待ち）。

`retry` を関数にして各 `npm install` を包んでいる。素で3回並べると、
1回目が成功したときも残り2回が走って時間を無駄にするため。

見張りは `src/test/iosSigningHardening.test.ts`。素の `npm install` が
戻ってきたら落ちる（変異検証で確認済み）。
