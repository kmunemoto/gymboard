-- Define helper to compute rank from avatar level
CREATE OR REPLACE FUNCTION public.compute_rank_for_level(_level integer)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    WHEN COALESCE(_level,1) <= 5  THEN 'rookie'
    WHEN _level <= 15 THEN 'regular'
    WHEN _level <= 30 THEN 'athlete'
    WHEN _level <= 50 THEN 'elite'
    ELSE 'legend'
  END
$$;

-- Replace process_session_rewards: per-rank 9x bonus, fix coin/ticket storage
...
DECLARE
  ...
  v_evo_def record;
  v_avatar_total_exp integer;
  v_avatar_new_level integer;
  v_required integer;
...
BEGIN
  ...
  SELECT COALESCE(SUM(exp_amount),0) INTO v_avatar_total_exp
  FROM public.avatar_exp_logs WHERE user_id = _user_id;
  v_cumulative := 0;
  v_lvl := 1;
  LOOP
    v_required := 250 + v_lvl * 50;
    IF v_avatar_total_exp < v_cumulative + v_required OR v_lvl > 999 THEN EXIT; END IF;
    v_cumulative := v_cumulative + v_required;
    v_lvl := v_lvl + 1;
  END LOOP;
  v_avatar_new_level := v_lvl;

  UPDATE public.user_avatars
  SET total_exp = v_avatar_total_exp,
      level = v_avatar_new_level,
      coins = COALESCE(coins,0) + v_coins,
      updated_at = now()
  WHERE user_id = _user_id;

  v_avatar_lvl_after := v_avatar_new_level;
  ...
  IF v_rank_after IS DISTINCT FROM v_rank_before THEN
    v_rank_up := true;
    CASE v_rank_after
      WHEN 'regular' THEN v_rank_bonus_coins := 270;  v_rank_bonus_tickets := 18;
      WHEN 'athlete' THEN v_rank_bonus_coins := 450;  v_rank_bonus_tickets := 27;
      WHEN 'elite'   THEN v_rank_bonus_coins := 900;  v_rank_bonus_tickets := 45;
      WHEN 'legend'  THEN v_rank_bonus_coins := 1800; v_rank_bonus_tickets := 90;
      ELSE v_rank_bonus_coins := 0; v_rank_bonus_tickets := 0;
    END CASE;

    IF v_rank_bonus_coins > 0 THEN
      UPDATE public.user_avatars
    ...
    IF v_rank_bonus_tickets > 0 THEN
      FOR i IN 1..v_rank_bonus_tickets LOOP
        INSERT INTO public.user_gacha_tickets (user_id) VALUES (_user_id);
      END LOOP;
    END IF;

    INSERT INTO public.avatar_rank_up_rewards (user_id, rank_name, coins_awarded, tickets_awarded)
    VALUES (_user_id, v_rank_after, v_rank_bonus_coins, v_rank_bonus_tickets);
  END IF;
...
-- Retroactive grant for existing users based on current rank
DO $$
DECLARE
  r RECORD;
  v_rank text;
...
BEGIN
  FOR r IN
    SELECT ua.user_id, ua.level
    FROM public.user_avatars ua
  LOOP
    v_rank := public.compute_rank_for_level(COALESCE(r.level, 1));
    CASE v_rank
      WHEN 'regular' THEN v_bonus_coins := 70;   v_bonus_tickets := 17;
      WHEN 'athlete' THEN v_bonus_coins := 320;  v_bonus_tickets := 43;
      WHEN 'elite'   THEN v_bonus_coins := 1020; v_bonus_tickets := 87;
      WHEN 'legend'  THEN v_bonus_coins := 2620; v_bonus_tickets := 176;
      ELSE v_bonus_coins := 0; v_bonus_tickets := 0;
    END CASE;

    IF v_bonus_coins > 0 THEN
      UPDATE public.user_avatars
      SET coins = COALESCE(coins,0) + v_bonus_coins,
          updated_at = now()
      WHERE user_id = r.user_id;
    END IF;

    IF v_bonus_tickets > 0 THEN
      FOR i IN 1..v_bonus_tickets LOOP
        INSERT INTO public.user_gacha_tickets (user_id) VALUES (r.user_id);
      END LOOP;

      INSERT INTO public.avatar_rank_up_rewards (user_id, rank_name, coins_awarded, tickets_awarded)
      VALUES (r.user_id, v_rank || '_retroactive_9x', v_bonus_coins, v_bonus_tickets);
    END IF;
  END LOOP;
END $$;