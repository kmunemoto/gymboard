...
それが透過画像であること（背景つきのアイコンを流用していないこと）、各アセットの寸法、
ルートのマスターと `assets/icon-only.png` の一致、抽出元と出力先が別ファイルであること。
...
生成元は `src/assets/gymboard-logo.png`。`src/assets/gymboard-loader.png` として、
透明な余白を落として正方形の中央に置き直したものを書き出している
（ロゴ原本は余白が大きく上下の中心もずれているため、そのまま使うと小さく下寄りに出る）。
