...
```
背景（雪山・空・粉雪）をコードで描き、盾とGBを載せて上記5点＋Web/PWA用
（`public/icon-192.png` `icon-512.png` `apple-touch-icon.png` `favicon.png` `favicon.ico`）
を一括で書き出す。固定シードなので、流し直しても同じ絵になる。

**季節ごとにデザインを変えたい場合はこのファイルの定数を編集して流し直す。**

注意点:
- **抽出元は `assets/icon-emblem-src.png`。`assets/icon-only.png` を指定してはいけない**
  （出力先を読み直すことになり、流すたびに絵柄が劣化する）。
