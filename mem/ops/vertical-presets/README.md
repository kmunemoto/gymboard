...
> # ⚠️ 2026-08-03 で維持を終了しました
>
> 上流・下流の運用をやめたため、**このディレクトリは以後更新されません**
> （経緯は `mem/ops/vertical-fork.md` の冒頭）。
>
> 中身は **2026-08-03 時点のスナップショット**として残してあります。
> 各アプリが一度コピーして使う分には今でも有効です。ただし
> **上流が文言キーをリネームしても、もう誰も追従しません。**
> `src/test/verticalPresets.test.ts` はジムボード側に残っているので、
> キーがずれれば上流のCIが落ちて気づけます（そのときは、そのプリセットが
> 古くなったというだけなので、落ちたら消すか直すか判断してください）。
