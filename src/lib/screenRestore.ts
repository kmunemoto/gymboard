 ...
 * ## 何が起きていたか
 *
 * 録画では、予約画面 → ブラウザで動画（約2秒）→ 戻る、で**アプリの起動画面（GBのロゴ）**が出て、
 * そのあとホームになっていた。起動画面が出る＝**アプリが最初から起動し直している**。
 * iOS は、前に出てきたアプリ（動画を流すブラウザなど）がメモリを要すると、
 * 裏に回ったアプリを**数秒でも終了させる**。これはアプリ側からは止められない。
 *
 * そして開いていたタブは React のメモリ（`useState("home")`）にしか無かったので、
 * 起動し直すと必ずホームに戻っていた。**止められないなら、覚えておいて戻す。**
 *
 * ## 何を戻すか・戻さないか
 *
 * | | |
