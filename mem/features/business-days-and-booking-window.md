...
### 選べる時間は1日の全域（2026-08-20 に広げた）

開店 `00:00`〜`23:30` / 閉店 `00:30`〜`24:00`、どちらも30分刻み。

それまでは **開店 07:00〜12:00 / 閉店 17:00〜23:00** に絞っていて、
実店舗から「選べる時間の範囲が狭い」と指摘された。
早朝のパーソナル・深夜営業・24時間営業はどれも珍しくないので、絞る理由が無い。
設定画面（`TrainerGymSettings`）と初回設定（`Onboarding`）で**同じ範囲**にすること。
曜日別も同じ配列を参照している（片方だけ広げると静かにズレる）。

### 🔴 `"24:00"` は終業だけの特別な値

24時間営業を表せるように、`parseTimeToMinutes("24:00")` は `1440` を返す。
`"23:59"` で代用すると枠の計算に半端な1分が混ざるので採らなかった。
**`"24:30"` や `"25:00"` は従来どおり無効**（`h > 23` の検査は残してある）。

⚠️ **ブロック枠の終了には `24:00` を出さない**（`blockEndMinutes` が
`24:00 − 刻み` で打ち切る）。ブロック枠は
`` `${日付}T${時刻}:00+09:00` `` の形で `blocked_slots` に保存されるが、
`"24:00"` は**翌日の 00:00 として保存され、読み戻すと `endTime` が `"00:00"`**
になる。すると重なり判定（開始 < 終了）が成立せず、
**そのブロックが何も塞がなくなる**。

なお**深夜またぎの営業（22:00〜翌02:00）には未対応**。
「終了は開始より後」の検査は残っている。必要になったら、枠の生成・
重なり判定・DBトリガーの全部に「日をまたぐ」概念を入れる必要がある。
