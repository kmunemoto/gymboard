...
ガイドライン 3.1.1（外部決済への誘導）で指摘される可能性がある。

- 旧の「Webでプランに申し込む」も外部リンクアウトで、**そちらは審査を通っている**
- ただし**Webのトップに飛ばす**のと**決済ページに直行させる**のとで見え方が変わりうる

**`featureFlags.ts` の `NATIVE_DIRECT_CHECKOUT` を false にすれば旧の形に戻る。**
サーバ側は何も変えなくてよい。リジェクトされたらフラグを倒して再提出する。

**Android から先に出すこと**（Play のほうが運用が緩く、手作業リリースなので戻しやすい）。
...
**ジムボードで審査が通ってから配る。** 通らなければ配らない。
配るときは `PRODUCTION_WEB_ORIGIN` と `STRIPE_LIVE_HOSTS` の対応を必ず確認すること
（片方だけ直すと sandbox に落ちる。`nativeCheckout.test.ts` が検出する）。
