...
| （次回） | **Play Console で確認** | **Play Console で確認** | 2026-08-13 に `android-version.json` を削除したので、**リポジトリからは現在値が読めない**。Android Studio で `android/app/build.gradle` を直接上げる。最後に Play へ上げた実績はこの表では **84 / 9.3（2026-08-09）まで確実**、85 / 86 は未確認（上の行の⚠️を参照）。🔴 **Android には PR #302（チャットの LINE 化）も #303（オーナーのアカウント削除）もまだ届いていない。** iOS は 1.5.4 と 1.5.5 で出したが、Android は 8/12 以降ビルドしていない。iOS の次の版数は **1.5.6**（1.5.5 が審査を通ってから上げる） |
