-- 閉店時に「その日だけ上限なし」も消す（2026-09-20）
--
-- booking_uncapped_days を足した（20260920010000）ので、delete_my_gym の
-- 消し込み対象に加える。
--
-- 🔴 定義は 20260902000100_delete_gym_booking_options.sql の**最新の版から機械的に写し**、
--    booking_uncapped_days の1行だけを足してある。古い版から書き直すと、その間に増えた
--    テーブルの消し込みが黙って消える（2026-08-21 に booking_capacity_windows で
--    実際に踏み、見張りテストが捕まえた）。
--    写す前に本番の pg_get_functiondef と突き合わせ、消し込み36件が一致することを確認済み。
...
BEGIN
  ...
  DELETE FROM public.booking_uncapped_days WHERE tenant_id = v_tenant_id;
