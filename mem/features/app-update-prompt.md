...
### 🔴 2026-09-20 — **上げずに保留**（宗本さんの判断）

`latest_version` は **ios = 1.7.0 / android = NULL のまま**。

その後 iOS は 1.7.2（Actions #154・#394 のキーボード修正）と
1.7.3（#155・#395 のブロック無視設定）をアップロード済みだが、
**どちらが公開済みか確認できていない**ため上げていない。
こちらから「上げますか」と聞いたところ「保留でお願いします」との回答（2026-09-20）。

⚠️ **この間、1.7.0 より新しい版を使っている人に更新のお願いは出ない。**
それは想定どおり（出す版が確定していないのに促すほうが危ない）。
再開するときは App Store Connect の実物を見て、**公開済みの版**を入れること。
`ios-build.yml` の `MARKETING_VERSION`（= 次に出す版）から採らないこと。
