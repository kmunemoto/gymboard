...
**いまの状態**: ★8枚すべて完成し、実装まで完了（2026-09-03）。
残りの8枚は絵ができ次第 `src/lib/stickers.ts` に足すだけ（マイグレーション不要）。

| # | id | 絵の文字 |
|---|---|---|
| 1 | `yoroshiku` | よろしくお願いします |
| 2 | `arigatou` | ありがとうございます |
| 3 | `ryokai` | 了解です！ |
| 4 | `ganbarimasu` | がんばります！ |
| 5 | `otsukaresama` | おつかれさま！ |
| 6 | `nice` | ナイス！ |
| 7 | `okuremasu` | ちょっと遅れます |
| 8 | `gomennasai` | ごめんなさい |

⚠️ `text` は**絵に描いてある文字と1字も違えない**こと。これがそのまま本文として
送られるので、ずれると古いアプリと新しいアプリで違うことを言っているように見える。
8枚とも1字ずつ目で確認済み（濁点の抜け・化けは無し）。
...
🔴 **手でやらない。`scripts/convert-sticker.py` を使う**（2026-09-03 に用意した）。

```bash
python3 scripts/convert-sticker.py ~/Downloads/nice.jpg src/assets/stickers/nice.png
```

- **透過 PNG**。生成は 1024×1024 以上、配布は **512×512** に縮小
- **上下左右に8%の余白**。目一杯だと窮屈に見える
- **影を落とさない**（背景が透過なので浮く）
- **輪郭線と文字の縁取りは太め**。チャットでは 128px で出る

🔴 **白を「色で」消さない。** スクリプトは外周から繋がっている白だけを塗りつぶしで
消している。「白なら透明」にすると、**文字の白い縁取り・目のハイライト・切り抜き風の
白フチまで消える**。特に縁取りが消えると、濃い面の上で文字が読めなくなる。

### 明暗どちらでも読めるか（2026-09-03 確認）

8枚を 128px にして、白（#FAFAFA）と濃色（#111318）の上に並べて目視した。

- **実際のチャットの面は白**（`--background: 30 20% 99%`）。写真背景にしても、
  チャットの外枠は `bg-background` で不透明なので、スタンプが濃い面に乗ることは
  いまは無い。**8枚とも問題なし**
- 参考: 仮に濃い面へ置くと、`ganbarimasu` と `okuremasu` は文字の白フチが細く
  読みづらい。濃色の面を作るときは、この2枚を縁取り太めで作り直すこと
...
```bash
python3 scripts/convert-sticker.py <生成した絵> src/assets/stickers/<id>.png
```

そのあと `src/lib/stickers.ts` の `STICKERS` に1行足すだけ。
**マイグレーションは要らない**（DB は id の形しか見ていない）。
`src/test/chatStickers.test.ts` が、透過・寸法・重さ・一覧との対応を見る。
