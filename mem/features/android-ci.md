...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。iOS は次 **1.6.4**（1.6.2 は欠番）。8/25・8/29 と両方を出したので、iOS と Android の中身は揃っている |
