-- 閉店時に動画ライブラリも消す（2026-08-31）
--
-- gym_videos を足した（20260831010000）ので、delete_my_gym の消し込み対象に加える。
-- 動画そのものは YouTube 側にあるので消えないが、URL と説明文はジムのものなので消す。
--
-- 🔴 定義は 20260826020000_delete_gym_email_log.sql の**最新の版から機械的に写し**、
--    gym_videos の1行だけを足してある。古い版から書き直すと、その間に増えた
