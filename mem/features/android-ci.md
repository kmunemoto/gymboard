...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。iOS は次 **1.6.2**。8/25 に iOS・Android の両方を出したので、**次回からは両者の中身が揃う**（8/12〜8/25 にあった大きなズレは解消済み）|
