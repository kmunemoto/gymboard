...
BEGIN
  ...
  -- New candidate occupies only its 60-min session. The 15-min buffer is forward-only
  -- and is already baked into existing bookings' end_at — adding it on both sides
  -- would wrongly block the slot before an existing booking.
  new_end := NEW.booking_date + interval '60 minutes';
