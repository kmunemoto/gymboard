...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。iOS は次 **1.6.6**（1.6.4 は未ビルドのまま 1.6.5 が先に出た。1.6.2 も欠番）。**Android は 8/29 以降ビルドしていない**ため、PR #363（動画ライブラリ）・#364（1日の受付上限・チャット修正）は Android には未到達 |
