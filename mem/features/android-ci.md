...
| （次回） | **Play Console で確認** | **Play Console で確認** | Android の版数はリポジトリで管理していない。Android Studio で `android/app/build.gradle` を直接上げる。🔴 **Android には #302 以降がすべて未到達**（8/12 以降ビルドしていない）。iOS は 1.5.7（#133）をビルド中。次に上げるのは 1.5.8 |
