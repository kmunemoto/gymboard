 ...
 * 常連さんを1人だけ足すのは店の裁量として残す。
 *
 * 🔴 **止まるのは「お客様の自己予約」だけ。** 公開の体験・ドロップイン予約は
 *    手で止めた日でも上限の日でも**止まらない**（下記）。
 *    ここには長らく「公開の体験・ドロップイン予約も止まる」と書いてあったが、
 *    **2026-09-19 に本番で実測したところ事実と違った**（体験は普通に入った）。
 *
 * ## 🔴 「手で止めた日も体験を止める」を入れて、同日に戻した（2026-09-19）
 *
 * 実測結果（赤くした日でも体験予約サイトからは申し込めた）をお見せしたところ、
 * 宗本さんは一度「体験も止める」を選び、PR #399 でその通りに入れた。
 * **しかし同じ日に「やっぱり赤い日も体験は予約入れられる様にして」で差し戻した**
 * （PR #399 の revert）。
 *
 * したがって現在の正は:
 *
 *   手で止めた日（赤）      … 会員の自己予約だけ止まる。体験・ドロップインは入る
 *   上限に達した日（オレンジ）… 会員の自己予約だけ止まる。体験・ドロップインは入る
 *
 * **体験・ドロップインはどちらでも止まらない。** 同じ提案をもう一度持ち込まないこと。
 * 変えるなら `trial_bookings` のトリガーと公開ページの両方を同時に、が条件
