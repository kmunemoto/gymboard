...
> # ⚠️ 2026-08-03 で更新を終了しました
>
> **上流・下流の運用そのものをやめた**ため、この一覧はここで止まります。
> 経緯と、やめたこと／やめていないことは `mem/ops/vertical-fork.md` の冒頭を参照。
>
> **上流で作業する人・エージェントへ: 以後ここに追記する必要はありません。**
> 下に並んでいるのは 2026-08-01〜03 の記録です。
> 兄弟アプリが最後に merge した時点によっては、まだ未対応の項目が残っている可能性がありますが、
> **追従するかどうかは各アプリの判断**になりました。
...
### 2026-08-02 (#239) `versionName` をワークフローに直書きにした
...
**⚠️ merge したら `ANDROID_VERSION_NAME` を自分のアプリの現在値に必ず直すこと。**
ジムボードの `9.1` がそのまま降ってくる。同じくテスト内の実績値
（`SHIPPED = [9, 0]`）も自分のアプリの値に直すこと。

**iOS と Android の版数を1本に統一するのは、やるなら数字が大きい側に寄せること。**
ジムボードでは一度統一しかけて取りやめた（iOS 1.4.5 / Android 9.0 で Android の方が
大きく、iOS に寄せると Android のお客様には「更新したらバージョンが戻った」に見える）。
**`versionName` は Play では順序を検査されないので、下げても弾かれずそのまま出る。**
App Store 側は増加を要求されるので事故が起きない＝**Android だけがノーガード**。
そのため「実績より下がっていないこと」をテストで見張っている。
